...
def selectThreshold(df, attribute, predict_attr):
    ...
    for i in range(0, len(values) - 1):
        ...
        ig = infoGain(df, attribute, predict_attr, thres)
        if ig > max_ig:
            max_ig = ig
            thres_val = thres
...
def remainder(df, df_subsets, predict_attr):
    ...
    for df_sub in df_subsets:
        if df_sub.shape[0] > 1:
            remainder += float(df_sub.shape[0] / num_data) * entropy(
                df_sub, predict_attr
            )
    return remainder
...
def infoGain(df, attribute, predict_attr, threshold):
    ...
    ig = entropy(df, predict_attr) - remainder(df, [sub_1, sub_2], predict_attr)
    return ig


# Returns the number of positive and negative data
def numberofClasses(df, predict_attr):
    p_df = df[df[predict_attr] == 1]
    n_df = df[df[predict_attr] == 0]
...
def chooseAttribute(df, attributes, predict_attr):
    max_info_gain = float("-inf")
    best_attr = None
    threshold = 0
    # Test each attribute (note attributes maybe be chosen more than once)
    for attr in attributes:
        thres = selectThreshold(df, attr, predict_attr)
        ig = infoGain(df, attr, predict_attr, thres)
        if ig > max_info_gain:
            max_info_gain = ig
            best_attr = attr
            threshold = thres
...
def buildTree(df, cols, predict_attr, max_depth=5):
    # Get the number of positive and negative examples in the training data
    p, n = numberofClasses(df, predict_attr)
    # If train data has all positive or all negative values
    # then we have reached the end of our tree
    if p == 0 or n == 0 or max_depth == 0:
        ...
        # information gain
        best_attr, threshold = chooseAttribute(df, cols, predict_attr)
        # Create internal tree node based on attribute and it's threshold
        tree = Node(best_attr, threshold)
        sub_1 = df[df[best_attr] < threshold]
        sub_2 = df[df[best_attr] > threshold]
        # Recursively build left and right subtree
        tree.left = buildTree(sub_1, cols, predict_attr, max_depth - 1)
        tree.right = buildTree(sub_2, cols, predict_attr, max_depth - 1)
        return tree
...
def test_predictions(root, df, predict_attr):
    num_data = df.shape[0]
    num_correct = 0
    for index, row in df.iterrows():
        prediction = predict(root, row)
        if prediction == row[predict_attr]:
            num_correct += 1
